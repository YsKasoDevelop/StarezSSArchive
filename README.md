...
配布物は[v1.02リリースページ](https://github.com/YsKasoDevelop/StarezSSArchive/releases/tag/v1.02)からダウンロードできます。ZIPをすべて展開し、フォルダ内の`StarezSSArchive.exe`を起動してください。.NETランタイムの別途インストールは不要です。
...
- 軽いノイズ除去と白い輪郭の抑制
- 初期状態では元画像を`original`フォルダへ同じ名前で移動し、処理後の画像を監視先へ保存
- 「オリジナルファイルを残さない」をONにすると原本保存を行わず、処理結果で元画像を安全に置換
- 指定フォルダ（初期値はSSフォルダ）の未処理PNGを設定画面から一括処理
- 監視先のPNGが100枚を超えた場合、古い画像を`backup`へ移動
- タスクトレイ常駐、SSフォルダを開く、設定、バージョン情報、終了
- 監視状態と処理状態を分けてタスクトレイに表示し、処理中のファイル名・処理待ち件数・直近の処理結果を確認可能
- タスクトレイから一時停止・再開が可能
- 監視先・一括処理先・バックアップ先・原本保存先・レタッチ・TTA・完了／エラー通知・原本の扱いを設定可能（通知と原本削除は初期状態OFF）
...
「オリジナルファイルを残さない」をONにした処理済み画像の記録は、次のファイルに保存されます。

    %LOCALAPPDATA%\StarezSSArchive\processed.json
...
- 設定：監視先、一括処理先、原本保存先、バックアップ保存先、レタッチ、TTA、完了／エラー通知、原本の扱い、自動起動を変更。変更時は処理待ちが終わるまで待ち、変更中に追加・更新されたPNGも切り替え後に確認します。「保存して一括処理」を押すと、指定フォルダ内の未処理PNGだけを処理します。
- バージョン情報：アプリ名とバージョン（1.02）を表示
- 終了：処理中の画像があれば完了を待って終了

レタッチをOFFにすると、新しく追加された画像の自動処理と一括処理を停止します。すでに処理待ちになっている画像は、現在の設定で処理が完了する場合があります。一括処理では、原本保存がONの場合は`original`フォルダに同名の原本がないPNG、原本保存がOFFの場合は処理済み記録に一致しないPNGを未処理として扱います。

## 動作仕様と注意

PNGの書き込み完了後に処理が始まります。通常は処理前の画像を`original`保存先に保持し、処理後の画像を元のフォルダに同じ名前で保存します。原本保存をOFFにした場合は、処理結果が完成してから元画像を置換します。AI処理に失敗した場合は元画像を残します。元画像と同じピクセル寸法で保存するため、AI処理によってファイルの寸法自体は変わりません。
...
監視の設定切替・候補保護・保存確定前の受付停止・一括処理の未処理判定は、AIエンジンを使わない隔離テストで確認できます。
